...
export class SatasupeActorSheet extends ActorSheet {
  ...
  get template() {
    if (this.actor.type === "npc") {
      ...
      if (!game.user.isGM && this.actor.limited) return "systems/satasupe/templates/actorlimit-sheet.html";
  ...
  activateListeners(html) {
    ...
    html.find('a.information').on("click", this._titleinformation.bind(this));

    html.find('.check-button').click(async (e) => {
      const actor = this.object;
      let d = await this._rollbutton(e);
      if (d.data.fumble) {
        let p = this._reversal(actor, d.data, d.card);
        await p.then(() => { })
      }
    }) //on("click", this._rollbutton.bind(this));
  ...
  async _reversal(actor, checkresult, card) {
    return new Promise((com) => {
      ...
      const p = new Promise((resolve, reject) => {
        const dlg = new Dialog({
          ...
          default: false,
          close: (e) => { reject(e); console.log("Close") },
        });
        dlg.render(true);
      })
      p.then(async (r) => {
        if (r) {
          let result = await this._bcdicesend(null, "1d6", null, null, game.i18n.localize("SATASUPE.UPSET"));
  ...
  async _rollbutton(event) {
    ...
    const actor = this.object.system;
    const copy = duplicate(this.object.system);
    let setting = { nobpwound: false, nompwound: false, nooverwork: false, killstop: false, nompcost: true, defdifficult: false };
    if (actor.settings) {
    ...
    if (canRoll) {
      ...
      const indata = await CheckDialog._createCheckdialog(char, value, setting, false);
      ...
      } else if (copy.attribs.bp.value <= 5) {
        bpwounds = 1;
      ...
      } else if (Number(copy.attribs.mp.value) <= 5) {
        mpwounds = 1;
      ...
      if (char != "alignment") copy.settings.defdifficult = Number(indata.get('difficulty'))
  ...
  async _bcdicesend(event, text, char, dif, message) {
    event?.preventDefault();
    const up = this;
