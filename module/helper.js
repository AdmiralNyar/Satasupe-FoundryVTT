...
export class EntitySheetHelper {
      ...
      if ( el.classList.contains("attribute-key") ) {
        ...
        for ( let i = 0; i < groups.length; i++ ) {
          if (groups[i].value == val) {
            ui.notifications.error(game.i18n.localize("SATASUPE.NotifyAttrDuplicate") + ` (${val})`);
            el.value = oldVal;
            attrError = true;
            break;
          }
  ...
  static validateGroup(groupName, entity) {
    ...
    if ( groups.includes(groupName) ) {
      ui.notifications.error(game.i18n.localize("SATASUPE.NotifyGroupDuplicate") + ` (${groupName})`);
      return false;
    }

    // Check for group keys that match attribute keys.
    if ( attributes.includes(groupName) ) {
      ui.notifications.error(game.i18n.localize("SATASUPE.NotifyGroupAttrDuplicate") + ` (${groupName})`);
      return false;
    }

    // Check for whitespace or periods.
    if ( groupName.match(/[\s|\.]/i) ) {
      ui.notifications.error(game.i18n.localize("SATASUPE.NotifyGroupAlphanumeric"));
      return false;
    }
  ...
  static async deleteAttributeGroup(event, app) {
    ...
    new Dialog({
      title: game.i18n.localize("SATASUPE.DeleteGroup"),
      content: `${game.i18n.localize("SATASUPE.DeleteGroupContent")} <strong>${group.val()}</strong>`,
      buttons: {
        confirm: {
