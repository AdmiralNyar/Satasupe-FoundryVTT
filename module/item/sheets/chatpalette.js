...
export class SatasupeChatpaletteSheet extends ItemSheet {
  ...
  async _allonoffToggle() {
    const system = duplicate(this.object.system);
    const nowonoff = system.status.allonoff;
    if (!nowonoff.value) {
      for (let i = 0; i < system.chatpalette.variable?.length; i++) {
        system.chatpalette.variable[i].substitution = true;
      }
    } else {
      for (let j = 0; j < system.chatpalette.variable?.length; j++) {
        system.chatpalette.variable[j].substitution = false;
      }
    }
    nowonoff.value = !nowonoff.value;
    await this.object.update({ 'system': system });
  }
      ...
      for (let i = 0; i < repal.length; i++) {
        ...
        for (let j = 0; j < item.system.chatpalette.variable.length; j++) {
          if ((item.system.chatpalette.variable[j].variable == repal[i]) && item.system.chatpalette.variable[j].substitution) {
            if (item.system.chatpalette.variable[j]?.sum) {
              text = text.replace(re, Number(item.system.chatpalette.variable[j].sum));
            } else {
